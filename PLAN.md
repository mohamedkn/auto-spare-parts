...
> نسخة معدّلة من v2 بعد مراجعة تانية ركّزت على جزئين: (1) تصليب هيكل قاعدة البيانات (Section 6) بإضافة indexes وON DELETE behavior وconstraints كانت ناقصة، (2) ترتيب برومپتات التنفيذ (Section 11) في صيغة code blocks جاهزة للنسخ بدل الفقرات المائلة. باقي الملف زي v2 بدون تغيير. الإضافات الجديدة مُعلّمة بـ 🆕v3. احتفظ بالملف في جذر الـ repo باسم `PLAN.md`.
>
> **🆕 تحديث لاحق (بعد v3):** إضافتين معلّمتين بـ 🆕 (بدون رقم نسخة، أحدث من v3): (1) تحقق إجباري إن أي إنشاء متجر مربوط بيوزر `role = 'vendor'` فعلاً (قسم 11، خطوة 3)، (2) تنظيم المعاملات المالية بشكل كامل — جداول `payouts` و`refunds` (قسم 6)، شروط استحقاق الـ payout والتعامل مع الاسترجاع (قسم 7.7 الجديد)، وخطوة تنفيذ منفصلة لهم بعد الـ MVP (قسم 11، خطوة 10).
...
-- 🆕 مالي: تسوية المتاجر (Payouts) — واحد بالظبط لكل sub_order
CREATE TABLE payouts (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  vendor_id UUID NOT NULL REFERENCES vendors(id) ON DELETE RESTRICT, -- 🆕 مالي: سجل مالي محمي
  sub_order_id UUID NOT NULL REFERENCES sub_orders(id) ON DELETE RESTRICT, -- 🆕 مالي
  amount DECIMAL(10,2) NOT NULL CHECK (amount > 0), -- 🆕 مالي
  status VARCHAR(20) NOT NULL DEFAULT 'pending'
    CHECK (status IN ('pending','processing','paid','failed')), -- 🆕 مالي
  payout_batch_id UUID, -- 🆕 مالي: تجميع دفعات التسوية الدورية (أسبوعي/شهري)
  bank_reference VARCHAR(100), -- 🆕 مالي: رقم التحويل البنكي كإثبات
  created_at TIMESTAMP DEFAULT now(),
  paid_at TIMESTAMP,
  UNIQUE (sub_order_id) -- 🆕 مالي: يمنع صراحة إنشاء payout مرتين لنفس sub_order
);
CREATE INDEX idx_payouts_vendor ON payouts(vendor_id); -- 🆕 مالي
CREATE INDEX idx_payouts_status ON payouts(status); -- 🆕 مالي
CREATE INDEX idx_payouts_batch ON payouts(payout_batch_id); -- 🆕 مالي

-- 🆕 مالي: استرجاعات جزئية/كاملة على مستوى عنصر الطلب
CREATE TABLE refunds (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  order_item_id UUID NOT NULL REFERENCES order_items(id) ON DELETE RESTRICT, -- 🆕 مالي
  payment_id UUID NOT NULL REFERENCES payments(id) ON DELETE RESTRICT, -- 🆕 مالي
  amount DECIMAL(10,2) NOT NULL CHECK (amount > 0), -- 🆕 مالي
  reason TEXT NOT NULL,
  status VARCHAR(20) NOT NULL DEFAULT 'pending'
    CHECK (status IN ('pending','approved','processed','rejected')), -- 🆕 مالي
  requested_by UUID NOT NULL REFERENCES users(id) ON DELETE RESTRICT, -- 🆕 مالي
  created_at TIMESTAMP DEFAULT now(),
  processed_at TIMESTAMP
);
CREATE INDEX idx_refunds_order_item ON refunds(order_item_id); -- 🆕 مالي
CREATE INDEX idx_refunds_payment ON refunds(payment_id); -- 🆕 مالي
```
...
> 🆕 **مالي — ليه payouts وrefunds جداول منفصلة مش مجرد حالة على sub_orders؟**
> - `sub_orders.vendor_payout_amount` بيحسب "قد إيه المفروض يتدفع للمتجر" وقت إنشاء الطلب، لكن **مفيش سجل فعلي إن الفلوس اتحولت فعلًا ولا لأ** بدون جدول `payouts` منفصل. الـ `UNIQUE(sub_order_id)` عليه تحديدًا عشان يمنع أي باج يعمل عملية تحويل مزدوجة لنفس الطلب.
> - `refunds` مربوطة بـ `order_item` مش `sub_order` كامل، عشان تدعم استرجاع جزئي (عميل رجّع منتج واحد من طلب فيه 3 منتجات) — ده سيناريو شائع جدًا وسياسة الاسترجاع (المذكورة في قائمة جاهزية الإطلاق) مش هتقدر تشتغل من غيره.
> - راجع القسم 7.7 تحت لتفاصيل **إمتى بالظبط** يتعمل payout، وإزاي refund بيأثر على الحساب.
...
### 🆕 7.7 مالي — شروط استحقاق الـ Payout والتعامل مع الاسترجاع

دي أكتر نقطة مالية عرضة للغلط في أي marketplace، ولازم تتحدد صراحة قبل أي كود:

**إمتى بالظبط يتعمل payout لمتجر؟** لازم الشروط الثلاثة دي مع بعض، مش شرط واحد:
1. `sub_order.status = 'delivered'` (المتجر نفّذ ونجح في التسليم)
2. **الفلوس فعلاً اتحصّلت** — يعني فيه `payment` مرتبط بنفس الـ `order` وحالته `succeeded`. ده مهم جدًا مع COD تحديدًا: التسليم مايعنيش تلقائيًا إن الفلوس وصلت حساب المنصة، لأن شركة الشحن ممكن تاخد وقت تحوّل الكاش المُحصّل. **متعملش payout بمجرد `delivered` لوحدها.**
3. **انقضاء نافذة الاسترجاع** — **3 أيام كاملة بعد `delivered_at`** (رقم صريح، قابل للتعديل من إعدادات المنصة لاحقًا، لكن مش placeholder مفتوح — الغموض في الرقم ده تحديدًا هو اللي أدى لتجاهله بالكامل في أول تنفيذ). عشان لو العميل رجّع المنتج بعد التسليم بيوم أو يومين، متبقاش مضطر تسحب فلوس اتدفعت للمتجر بالفعل.

**لو حصل refund قبل ما الـ payout يتعمل:** اخصم قيمة الـ `order_item` المسترجع من `sub_orders.vendor_payout_amount` قبل إنشاء سجل الـ payout (إعادة حساب، مش تعديل على `commission_amount` الأصلي — العمولة نفسها بترجع للعميل جزئيًا أو بتتحمّلها المنصة، قرار عملي يحتاج توثيق صريح في الكود كتعليق).

**لو حصل refund بعد ما الـ payout اتعمل بالفعل:** الحالة دي أعقد (استرداد فلوس من متجر اتدفعله بالفعل) — أجّلها لمرحلة النزاعات (Disputes، Phase 3) بدل ما تحاول تحلها آليًا في Phase 2؛ سجّلها كـ dispute مربوط بالـ sub_order.
...
> 🆕 **مالي:** الموديل الثاني (تحصيل مركزي) هو اللي جدول `payouts` في القسم 6 مبني عليه، وشروط تشغيله (إمتى بالظبط يتعمل الـ payout الفعلي) موثّقة بالتفصيل في **القسم 7.7**. لو قررت لاحقًا (Phase 3+) تنتقل لموديل Split، جدول `payouts` هيتغير دوره من "سجل تحويل يدوي/دفعي" لـ "سجل تأكيد من الـ payment provider" — لكن مش لازم تفكر في ده دلوقتي.
...
- 🆕 **مالي — Cash on Delivery:** حتى لو COD، اعمل سجل `payment` (provider = `'cod'`) وقت إنشاء الطلب بحالة `pending`. **متحدّثش الحالة لـ `succeeded` عند التسليم تلقائيًا** — خليها تتحدث لما يتأكد استلام الكاش فعليًا (يدويًا من الأدمن أو المتجر في البداية). ده الشرط اللي شروط الـ payout في القسم 7.7 بتعتمد عليه؛ من غيره هتدفع للمتاجر عمولات على فلوس لسه ماوصلتش المنصة.
...
مهم: أي endpoint بينشئ سجل في جدول vendors (مثلاً POST /api/vendor/register)
لازم يتحقق أولًا إن req.user.role === 'vendor' قبل أي INSERT، وإلا يرجع
403 Forbidden. هذا تحقق application-level إجباري لأن Postgres CHECK
constraint مايقدرش يتحقق من جدول تاني (users) وقت الـ INSERT على vendors.

لا تلمس أي موديول تاني.
```

**✅ راجع:** الأدوار الثلاثة شغالة، الـ validation موجود، الباسورد بيتعمله hash صح، وإنشاء متجر مرفوض لأي يوزر role مش 'vendor'.
...
### 🆕 الخطوة 10 — Phase 2: Payouts وRefunds (تنفّذ بعد ما الـ MVP كامل ومُختبر فقط)

هذه الخطوة **مش** جزء من Phase 1 — نفّذها بس بعد إتمام واختبار الخطوات 0-9 فوق، وبعد ما بوابة الدفع الحقيقية (القسم 10) شغالة.

> ⚠️ **تصحيح موثّق (بعد أول محاولة تنفيذ):** أول تنفيذ للخطوة دي انحرف عن السبك تحت في 3 نقاط: (1) استخدم عمود `PayoutStatus` enum على `sub_orders` بدل جدول `payouts` منفصل، (2) الـ eligibility check اعتمد بس على `status = 'delivered'` من غير التحقق من `payment.status = 'succeeded'` (خطر حقيقي مع COD)، (3) `refunds` متعملتش خالص. الـ prompt تحت اتحدّث ليطلب تصحيح صريح، مش تنفيذ من الصفر.

```
راجعنا التنفيذ الحالي للـ payouts ولقينا 3 انحرافات عن القسم 7.7 في
PLAN.md لازم تتصحح. متبدأش من الصفر — عدّل الموجود:

1. Migration من enum لجدول منفصل:
   - اعمل migration جديدة تنشئ جدول `payouts` (id, vendor_id FK RESTRICT,
     sub_order_id FK RESTRICT UNIQUE, amount, status enum
     pending/processing/paid/failed, payout_batch_id nullable,
     bank_reference nullable, created_at, paid_at nullable).
   - Backfill: لأي sub_order حاليًا PayoutStatus = 'paid'، اعمل سجل payout
     مقابل بـ amount = vendor_payout_amount، status = 'paid'،
     paid_at = updatedAt الحالي بتاع الـ sub_order (تقريبي لأننا معندناش
     التاريخ الحقيقي، وده مقبول لأنها بيانات اختبار).
   - بعد الـ backfill، احذف عمود PayoutStatus من SubOrder model، وحدّث كل
     مكان في الكود كان بيقرأ منه ليقرأ من وجود/حالة سجل في payouts بدل كده.

2. Eligibility check — لازم الثلاثة شروط مع بعض مش شرط واحد:
   - sub_order.status = 'delivered'
   - فيه payment مرتبط بنفس order وحالته 'succeeded' (مش 'pending' ولا
     'pending_verification' — ده بالذات اللي كان ناقص، وخطير مع COD)
   - عدّى على الأقل 3 أيام كاملة من delivered_at (استخدم قيمة ثابتة
     دلوقتي، مش لازم تبقى configurable في المرحلة دي)
   - Endpoint الأدمن (GET /api/admin/payouts) لازم يعرض بس sub_orders
     اللي عدّت الشروط التلاتة، مش delivered بس.
   - عند التسديد (POST)، استخدم الـ UNIQUE(sub_order_id) كحماية: لو حصل
     محاولة إنشاء payout تاني لنفس sub_order، لازم يفشل بوضوح مش يتجاهل
     بصمت.

3. Refunds من الصفر:
   - أضف جدول refunds (id, order_item_id FK RESTRICT, payment_id FK
     RESTRICT, amount, reason, status enum
     pending/approved/processed/rejected, requested_by FK RESTRICT,
     created_at, processed_at).
   - Endpoint للأدمن ينشئ refund على order_item معيّن، بشرط: لو فيه سجل
     payout موجود بالفعل لنفس sub_order، ارفض العملية بـ رسالة واضحة
     ("لازم تتحول لـ dispute")، ومتنشئش الـ refund.
   - لو مفيش payout لسه، اسمح بالـ refund واخصم قيمته من
     sub_orders.vendor_payout_amount قبل أي حساب payout مستقبلي.

اطلب توضيح أي جزء غامض قبل ما تبدأ، ومتلمسش أي موديول تاني.
```

**✅ راجع (لا تتجاوز هذه الخطوة بدون تأكد):**
- [ ] جدول `payouts` موجود فعليًا، والـ enum القديم اتشال من SubOrder
- [ ] بيانات الاختبار القديمة (لو موجودة) اتنقلت صح للجدول الجديد
- [ ] جرّب تعمل sub_order بحالة `delivered` بس والـ payment لسه `pending` (زي COD قبل التحصيل) — لازم **ميظهرش** في قائمة الأدمن المستحقة
- [ ] جرّب نفس السيناريو بعد ما تحدّث الـ payment لـ `succeeded` لكن قبل 3 أيام من `delivered_at` — لازم **ميظهرش** برضه
- [ ] بعد 3 أيام + `succeeded` + `delivered` مع بعض — لازم **يظهر** كمستحق
- [ ] جرّب تشغّل POST تسديد مرتين على نفس sub_order — لازم يفشل التاني (`UNIQUE`)
- [ ] جرّب refund بعد وجود payout بالفعل — لازم يترفض ويوجّه لمسار dispute
- [ ] `vendor_payout_amount` بيتحدّث صح لو حصل refund جزئي قبل الـ payout
